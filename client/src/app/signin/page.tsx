...
const SigninPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}auth/signin`, { email, password });
  ...
  return (
    <div className="bg-background flex h-full min-h-screen w-full flex-col items-center justify-center px-8 py-16">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg">
        <h2 className="text-center text-2xl font-bold font-mono">Signin</h2>
        <p className="text-center text-sm italic">Please sign in to take note</p>
